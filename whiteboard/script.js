    ...
    if (file && file.type.startsWith("image/")) {
        ...
        reader.onload = function (e) {
            ...
            img.onload = function () {
                ...
                clearCanvas(); // Clear the canvas and draw all images
            };
...
// Function to check if mouse is over any image resize corner
function isMouseOverResizeCorner(mouseX, mouseY, img) {
    const cornerX = img.x + img.width - 10;
    const cornerY = img.y + img.height - 10;
    return mouseX >= cornerX && mouseX <= cornerX + 10 && mouseY >= cornerY && mouseY <= cornerY + 10;
}

// Function to check if mouse is over an image to drag it
function isMouseOverImage(mouseX, mouseY, img) {
    return mouseX >= img.x && mouseX <= img.x + img.width && mouseY >= img.y && mouseY <= img.y + img.height;
}

// Function to start resizing or dragging
function startResizingOrDragging(e) {
    ...
    for (let i = 0; i < images.length; i++) {
        ...
        if (isMouseOverResizeCorner(mouseX, mouseY, img)) {
            activeImageIndex = i;
            isResizing = true;
            return;
        }

        if (isMouseOverImage(mouseX, mouseY, img)) {
            activeImageIndex = i;
            isDragging = true;
            return;
        }
...
// Function to resize or drag the selected image
function resizeOrDragImage(e) {
    if (isResizing && activeImageIndex >= 0) {
        const img = images[activeImageIndex];
        img.width = e.offsetX - img.x;
        img.height = e.offsetY - img.y;
        clearCanvas(); // Clear the canvas and redraw images
    }

    if (isDragging && activeImageIndex >= 0) {
        const img = images[activeImageIndex];
        img.x = e.offsetX - img.width / 2;
        img.y = e.offsetY - img.height / 2;
        clearCanvas(); // Clear the canvas and redraw images
    }
}

// Function to stop resizing or dragging
function stopResizingOrDragging() {
    isResizing = false;
    isDragging = false;
}
...
function loadFile(drawingData, notes, backgroundColor, images) {
 ...
 // Load and display saved notes
        async function loadSavedNotes() {
            ...
            savedNotesContainer.innerHTML = "<h3>Saved Notes</h3>";  // Reset saved notes list
...
function interact(e) {
    ...
    if (mode === 'pen') {
        ctx.lineWidth = brushSize;
        ctx.lineCap = "round";
        ctx.strokeStyle = mode === "erase" ? "#ffffff" : color;  // Use white if erasing
        ctx.lineTo(e.offsetX, e.offsetY);
        ctx.stroke();
    }
}
...
function loadFile(drawingData, notes, backgroundColor, images) {
 ...
 async function loadSavedNotes() {
    ...
    try {
        ...
        if (data) {
            Object.keys(data).forEach(key => {
                const entry = data[key];  // Get each saved entry (notes, notesColor)

                // Create a div to display the saved note
                const noteDiv = document.createElement("div");
                noteDiv.classList.add("saved-note");
                noteDiv.style.color = entry.notesColor;  // Apply the saved notes color
                noteDiv.textContent = entry.notes;  // Set the note text

                savedNotesContainer.appendChild(noteDiv);  // Add the note div to the saved notes container
            });
        }
    } catch (error) {
